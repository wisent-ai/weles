// Which credential each Weles service owns, and which acquired secrets have a
// writer and a reader.
//
// Moved verbatim out of the single scoped-service file during a split by
// responsibility. Nothing here reaches the authority; these are declarations
// the rest of the family is measured against.
...
export function resolvedAcquiredSecretContract(secret: string): InternalAcquiredSecretContract | null {
  ...
  // stays the only authority that can enable it — there is no second consumer or
  // token to try, and no reader consumer, so the derived contract can never be
  // read back. An item another contract already owns is refused here: a derived
  // contract must never reach a table item's or a scoped service item's fields.
  // An id that merely looks like a managed password lands here too, with no
  // source origin and no reader: only the table above may hand out a provider
  // surface, so a renamed or unenumerated item fails visibly at the reader and
  // shape gates instead of silently inheriting Microsoft's.
