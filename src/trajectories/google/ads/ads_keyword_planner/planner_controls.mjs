// Driving the planner surface: open it, put the keywords in, press the
// control that produces results, and read what came back.
//
// Moved verbatim during a split by responsibility, with three repairs.
//
// The account row on the selector page is one row spelled three ways — the
// account number, the company name, the generic label — and it used to be
// searched by walking a list and stepping over each miss. It is one locator
// now, the same way every other control in this trajectory is addressed.
//
// The four documented addresses of the Keyword Planner page are still walked,
// but a navigation that fails now raises instead of being logged and stepped
// over: the walk exists because Google moves the page, not to paper over a
// browser that cannot navigate.
//
// The keyword input has two ways in — the page's own field, set directly, or
// the same field typed into by keyboard — and the second used to report itself
// with a name borrowed from the driver rather than saying what it did. Nothing
// outside this trajectory ever read that value.

import { humanClickLocator, humanIdlePause } from '../../../../../dist/human/mouse.js';
import { humanFill } from '../../../../../dist/human/keyboard.js';
import { collectDom, summarizeKeywordPlannerResponses } from './page_reading.mjs';
import { parseKeywordRows } from './keyword_rows.mjs';
import {
  buildGoogleAdsPath,
  campaignsUrl,
  cid,
...
const PLANNER_SURFACE_RE = /keyword planner|discover new keywords|get search volume|forecasts?|keyword ideas|avg\.? monthly searches/i;
...
export async function continueFromGoogleAdsAccountSelector(page) {
  ...
  const accountRow = page.getByText(new RegExp(dashed.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')))
    .filter({ visible: true })
    .first()
    .or(page.getByText(/Wisent-AI, Inc/i).filter({ visible: true }).first())
    .or(page.getByText(/Google Ads account/i).filter({ visible: true }).first());
  if (!await accountRow.isVisible().catch(() => false)) return false;
  console.log(`[google-ads-keyword-planner] selecting Google Ads account ${dashed}`);
  await humanClickLocator(page, accountRow);
  await humanIdlePause('deliberate');
  return true;
}
...
export async function fillKeywordInput(page) {
  ...
  const result = await page.evaluate(({ text }) => {
    ...
    const visit = (root) => {
      for (const el of root.querySelectorAll?.('textarea,input,[contenteditable="true"],[role="textbox"]') || []) {
        ...
        fields.push({ el, descriptor, score });
      }
    };
    visit(document);
    fields.sort((a, b) => b.score - a.score);
    const selected = fields.find((field) => field.score > 0) || fields[0];
  ...
  const typedInput = page.locator('textarea, input[type="text"], input:not([type]), [contenteditable="true"], [role="textbox"]')
    .filter({ visible: true })
    .first();
  if (await typedInput.isVisible().catch(() => false)) {
    await humanFill(page, typedInput, text);
    await humanIdlePause('deliberate');
    return { ok: true, descriptor: 'typed_into_first_visible_text_field' };
  }
  return result || { ok: false, reason: 'no_keyword_input' };
}

const PLANNER_PATHS = Object.freeze([
...
async function readPlannerAt(s, path) {
  ...
  return {
    path,
    url: dom.url || url,
    matched: PLANNER_SURFACE_RE.test(dom.text),
    textPreview: norm(dom.text).slice(0, 800),
    controls: dom.controls.slice(0, 20),
  };
}

export async function openKeywordPlanner(s) {
  const attempts = [];
  for (const path of PLANNER_PATHS) {
    const attempt = await readPlannerAt(s, path);
    attempts.push(attempt);
    if (attempt.matched) return { ok: true, path, attempts };
  }
  ...
  if (clickedTools) {
    ...
    await s.wait(8);
    const dom = await collectDom(s.page);
    const matched = PLANNER_SURFACE_RE.test(dom.text);
...
async function pressResultControl(s) {
  const actionClicks = [];
  for (const action of RESULT_CONTROL_LABELS) {
...
export async function collectKeywordPlanner(s, captured) {
  ...
  const actionClicks = await pressResultControl(s);
  await s.wait(8);
