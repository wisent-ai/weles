...
for (let retry = 0; retry < 3; retry++) {
  try {
    ...
    try {
      const all = loadFreshCookieJarOrFail(acct, { platform: 'github', label: 'github_star', currentProxyUrl: proxyUrl, currentPersona: persona });
...
try {
  ...
  else { console.log(`FAIL: star did not register`); process.exitCode = 1; }
} catch (e) {
