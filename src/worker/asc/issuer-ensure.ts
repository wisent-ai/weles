...
import { DATABASE_URL, headers } from '../stale.js';
...
async function rows(q: string): Promise<unknown[]> {
  const r = await fetch(`${DATABASE_URL}/rest/v1/account_action_logs?${q}`, { headers: headers() });
...
async function enqueue(row: Record<string, unknown>): Promise<void> {
  const r = await fetch(`${DATABASE_URL}/rest/v1/account_action_logs`, { method: 'POST', headers: { ...headers(), Prefer: 'return=minimal' }, body: JSON.stringify(row) });
...
export async function ensureAscIssuer(): Promise<void> {
  if (process.env.WELES_ENSURE_ASC_ISSUER === '0' || !DATABASE_URL) return;
  try {
