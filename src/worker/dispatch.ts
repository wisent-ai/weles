...
const ROUTES: Record<string, (p: string) => string | null> = {
  ...
  login: (p) => {
    ...
    // Subscription logins live in their provider's directory next to that
    // provider's reauth trajectory. Without this branch <provider>_login
    // resolved to a top-level file that does not exist, so the only way to run
    // one was reauth's own LRU pick — the queue could not ask for a login at all.
    if (p === 'claude' || p === 'codex' || p === 'kimi') return `scripts/trajectories/${p}/login.mjs`;
