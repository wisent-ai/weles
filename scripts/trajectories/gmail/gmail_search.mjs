...
//
// Pattern mirrors scripts/trajectories/google/gcp_credits.mjs and
// scripts/trajectories/_shared/services/setup_chrome_profile.mjs: it uses the
...
//
// Run:  node scripts/trajectories/gmail/gmail_search.mjs
...
try {
  ...
  if (status !== 'in') {
    log('FAIL: profile not signed in to Gmail (status=' + status + ', url='
      + s.page.url() + ').');
    log('Fix: run scripts/trajectories/_shared/services/setup_chrome_profile.mjs');
