...
const t = TRAJECTORIES.find(t => t.name === 'linkedin_register');
if (!t) { console.error('Trajectory not found: linkedin_register'); process.exit(1); }
if (t.emailEnv) {
...
const s = await WSession.start({ label: 'linkedin_register', proxy: process.env.PROXY_URL || undefined });
try {
  await s.goto(t.url);
  const result = await execute(s.page, `Open ${t.url}. ${t.goal}`, {
    envHints: t.emailEnv ? { SVC_EMAIL: process.env.SVC_EMAIL, SVC_PASSWORD: '***' } : {},
    flowName: 'linkedin_register',
  });
