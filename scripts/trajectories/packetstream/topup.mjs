...
try {
  ...
  if (/paypal\.com|stripe\.com|checkout/i.test(finalUrl)) {
    console.log(`PASS-CHARGED-PENDING: redirected to checkout (${finalUrl}). PayPal/Stripe will complete the $${target} charge against the saved payment method. Verify via PacketStream's "Recent Payments" or by re-running scripts/trajectories/packetstream/balance.mjs in 1-2 minutes.`);
    process.exit(0);
  }
