...
const GOAL = `Fill email with $SVC_EMAIL. Fill password with $SVC_PASSWORD. Click "Log in". Wait for redirect. done(value="logged in").`;
...
const s = await WSession.start({ label: 'tiktok_login', proxy: process.env.PROXY_URL || undefined });
try {
  await s.goto(URL);
