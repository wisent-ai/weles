...
try {
  ...
  writeFileSync(`${OUT_DIR}/${stamp()}_reset_text.txt`, text);
  ...
  console.log(`[trajectory] full username from page text: ${fullUsername}`);

  // Also try to read input[name="username"] / specific fields if present
  const inputs = await s.page.evaluate(() => Array.from(document.querySelectorAll('input')).filter(i => i.offsetParent).map(i => ({ name: i.name, id: i.id, value: i.value, type: i.type, ph: i.placeholder })));
  ...
  await shot(s, 'after_gen_click');

  // If a password input becomes editable, type the new password.
  const passInputs = await s.page.evaluate(() => Array.from(document.querySelectorAll('input')).filter(i => i.offsetParent && (i.type === 'password' || /password/i.test(i.name + i.id + i.placeholder))).map(i => ({ name: i.name, id: i.id, value: i.value })));
  console.log(`[trajectory] password inputs after gen: ${JSON.stringify(passInputs)}`);
  ...
  if (passInputs.length > 0) {
    ...
    if (await pwLoc.isVisible().catch(() => false)) {
      ...
      if (await confirmBtn.isVisible().catch(() => false)) {
        ...
        await humanIdlePause('long');
        await shot(s, 'after_pw_set');
      }
    }
  }

  // 3. Final snapshot of credentials displayed
  const finalText = await s.page.evaluate(() => (document.body && document.body.innerText || '').slice(0, 8000));
  writeFileSync(`${OUT_DIR}/${stamp()}_final_text.txt`, finalText);
  await shot(s, 'final_state');
  ...
  if (fullUsername) {
    ...
    console.log(`[trajectory] wrote oxylabs_isp_creds.json username=${fullUsername} pw_len=${NEW_PASSWORD.length}`);
    // Probe :8001
