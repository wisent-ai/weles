...
import { loginFromSkarbiec, requireGraphicalSession } from '../_shared/reauth_config.mjs';
...
try {
  // Before the Kimi CLI is spawned and a browser is asked for a window: this
  // run needs a graphical session, and saying so here costs three minutes less
  // than discovering it when Google's popup takes the browser down with it.
  if (process.env.KIMI_LOGIN_HEADLESS !== '1') requireGraphicalSession('the Kimi login');
  mkdirSync(VAR, { recursive: true });
