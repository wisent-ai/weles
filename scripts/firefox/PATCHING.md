...
- [~] **P2.6** Release pipeline. **Consumer side shipped:** `findCustomChromium` extracted into `src/session/find_browser.ts` as `findCustomBrowser(browser)`; WSession scans `~/.local/share/weles-firefox/` and `../firefox-build/obj-weles/dist/` when `browser === 'firefox'`. `scripts/firefox/download.sh` mirrors the Chromium downloader (exits 2 until `WELES_FIREFOX_RELEASE` points at a real tag). **Producer side shipped:** `firefox-build/build.sh` wraps bootstrap + patch apply + mach build idempotently; `firefox-build/release.sh` tarballs the output into `weles-firefox-<version>-<plat>.tar.gz`. **First artifact built 2026-04-23:** `weles-firefox-142.0a1-weles.1-macos-arm64.tar.gz` sha256 `9766b2041e366aa342cba62076e3bc614bf88dda0f476ee8a5e34402c34386d1`, 91 MB. **Still TBD:** creating `wisent-ai/weles-firefox` repo and uploading — user-side GitHub action.
...
- [ ] **P3.2** Side-by-side JA4 / canvas / BotD / creepjs / amiunique capture vs real Firefox Nightly. The Phase 1 `capture_fingerprint_local.mjs` uses Playwright which can't drive our non-juggler binary; port the HTTP-round-trip trick from `verify.mjs` into a capture harness, then diff.
