...
- [~] **P2.6** Release pipeline. **Consumer side shipped:** `findCustomChromium` extracted into `src/session/find_browser.ts` as `findCustomBrowser(browser)`; WSession scans `~/.local/share/weles-firefox/` and `../firefox-build/obj-weles/dist/` when `browser === 'firefox'`. `scripts/firefox/download.sh` mirrors the Chromium downloader (exits 2 until `WELES_FIREFOX_RELEASE` points at a real tag). **Producer side partially shipped:** `firefox-build/build.sh` wraps bootstrap + patch apply + mach build idempotently; `firefox-build/release.sh` tarballs the output into `weles-firefox-<version>-<plat>.tar.gz` with the SHA-256 for GitHub Release attachment. **Still TBD:** creating `wisent-ai/weles-firefox` repo and uploading the first tarball — user-side GitHub action.
