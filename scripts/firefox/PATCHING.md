...
- [x] **P3.3** CI auto-probe. `.github/workflows/firefox-integration.yml` runs on `workflow_dispatch` (manual: `gh workflow run firefox-integration.yml -f tag=firefox-142.0a1-weles.N`) or `repository_dispatch firefox-release-published`. macOS-15 runner downloads the tarball from wisent-ai/weles-firefox, extracts into `~/.local/share/weles-firefox/<version>/`, sets `WELES_FIREFOX_RELEASE`, then runs `scripts/firefox/integration_test.mjs`. Non-zero exit fails the workflow. Manual-trigger only keeps runner cost low.
