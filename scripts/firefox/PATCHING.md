...
- [~] **P2.2** `navigator.webdriver` at engine level. Patch spec in `patch_sketches.md` P2.2 — insert `weles.fingerprint.webdriver.force` read at the top of `Navigator::Webdriver()` in `dom/base/Navigator.cpp`. Juggler-side injection path may also need a second site (check `testing/mozbase/mozrunner/.../juggler/content/main.js` at build time).
- [~] **P2.3** WebGL vendor/renderer un-normalized. Patch spec in `patch_sketches.md` P2.3 — insert `weles.fingerprint.webgl.{vendor,renderer}` read in `dom/canvas/WebGLContext.cpp::GetParameter` before Firefox's `SanitizeVendorString`.
- [~] **P2.4** Screen overrides. Patch spec in `patch_sketches.md` P2.4 — insert `weles.fingerprint.screen.{width,height,avail_width,avail_height}` read at the top of each getter in `dom/base/Screen.cpp`.
- [~] **P2.5** Window-outer overrides. Patch spec in `patch_sketches.md` P2.5 — insert `weles.fingerprint.window.{outer_width,outer_height,screen_x,screen_y}` read at the top of each getter in `dom/base/nsGlobalWindowOuter.cpp`.
