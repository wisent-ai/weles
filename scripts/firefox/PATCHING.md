...
- [~] **P3.3** CI auto-probe. `.github/workflows/firefox-integration.yml` runs on `workflow_dispatch` (manual: `gh workflow run firefox-integration.yml -f tag=firefox-142.0a1-weles.N`) or `repository_dispatch firefox-release-published`. macOS-15 runner downloads the tarball, extracts, sets `WELES_FIREFOX_RELEASE`, runs `scripts/firefox/integration_test.mjs`. **Status 2026-04-24:** workflow YAML valid + cross-repo auth working (via `WELES_FIREFOX_TOKEN` secret) + binary extraction + direct `firefox --version` PASSES on runner. But Playwright's `firefox.launch(executablePath)` + `newContext` + `newPage` dies with "Target page, context or browser has been closed" on the VMAPPLE-backed `macos-15` runner — the juggler-pipe protocol handshake fails in the VM environment even though identical tarball + client version works on native macOS. Local `integration_test.mjs` PASSES. Runner-env regression; separate from the Firefox parity closure itself. Companion weles-firefox repo has an auto-dispatch workflow (`notify-weles-on-release`) that will fire this workflow on every future release; the `WELES_WELES_DISPATCH_TOKEN` secret is set.
