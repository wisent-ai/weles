...
async function capture(label) {
  ...
  try {
    ...
    console.log(`\n${label}:`);
    console.log('  ja4:', json.ja4);
    console.log('  ja4_fingerprint:', json.ja4_fingerprint);
    console.log('  tls:', json.tls);
  } catch {
    console.log(label, 'raw:', raw.slice(0, 500));
  }
}

await capture('before-linkedin');
await page.goto('https://www.linkedin.com/signup', { waitUntil: 'domcontentloaded', timeout: 30000 });
await new Promise(r => setTimeout(r, 3000));
await capture('after-linkedin');
await ctx.close();
