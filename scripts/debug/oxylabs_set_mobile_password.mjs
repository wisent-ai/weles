// Set Oxylabs Mobile sub-user password to a known test value and verify proxy auth.
...
const s = await WSession.start({ label: 'oxylabs_set_mobile_password', browser: 'chromium' });
try {
  ...
  await s.goto('https://dashboard.oxylabs.io/en/overview/MP/users');
  await humanIdlePause('long');
  await s.page.screenshot({ path: join(OUT, '00_users.png'), fullPage: true });

  const editBtn = s.page.locator('tr').filter({ hasText: 'userinho_sHJzQ' }).locator('button, a').filter({ hasText: /Edit user|Change password/i }).first();
  ...
  const input = s.page.locator('input[type="text"][name="password"]').filter({ visible: true }).first();
  await input.fill('');
  await input.fill(TEST_PASSWORD);
  await humanIdlePause('short');

  // List all visible buttons before saving.
  ...
  // Test proxy auth with new password.
  ...
  const pres = await probe({ host: 'pr.oxylabs.io', port: 7777, username: 'customer-userinho_sHJzQ-cc-US', password: TEST_PASSWORD });
