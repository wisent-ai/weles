...
function artifact(value, name) {
  ...
  if (url.protocol !== 'https:') throw new Error(`${name}.url must use HTTPS`);
  ...
  if (provenance.protocol !== 'https:') throw new Error(`${name}.provenanceUrl must use HTTPS`);
...
export function validateManifest(value) {
  ...
  if (!SEMVER.test(text(manifest.worker?.version, 'worker.version'))) throw new Error('worker.version must be SemVer');
  revision(manifest.worker?.sourceRevision, 'worker.sourceRevision');
  if (!Array.isArray(manifest.worker?.artifacts) || !manifest.worker.artifacts.length) throw new Error('worker.artifacts is empty');
  manifest.worker.artifacts.forEach((item, index) => artifact(item, `worker.artifacts[${index}]`));
  const web = object(manifest.web, 'web');
  ...
  const client = object(manifest.client, 'client');
  ...
  for (const family of ['chromium', 'firefox']) {
    const browser = object(manifest.browsers?.[family], `browsers.${family}`);
    text(browser.release, `browsers.${family}.release`);
  ...
  const schemaVersion = Number(manifest.database?.schemaVersion);
  if (!Number.isInteger(schemaVersion) || schemaVersion < 1) throw new Error('database.schemaVersion must be a positive integer');
  exactSha(manifest.database?.migrationSetSha256, 'database.migrationSetSha256');
  const workerRange = versionRange(manifest.compatibility?.workerDatabase, 'compatibility.workerDatabase');
  const webRange = versionRange(manifest.compatibility?.webDatabase, 'compatibility.webDatabase');
...
export async function installArtifact(options) {
  ...
  try {
    ...
    if (record.sha256 !== selected.sha256 || record.manifestSha256 !== manifestSha256) {
      throw new Error(`installed ${component} destination has different provenance`);
    }
