// CRUD helper for the service_subscriptions table via Supabase REST.
...
export async function listSubscriptions({ service, status, provider, account } = {}) {
  ...
  const res = await fetch(`${SUPABASE_URL}/rest/v1/service_subscriptions?${params.toString()}`, {
    method: 'GET',
    headers: apiHeaders(),
  });
...
export async function upsertSubscription(row) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/service_subscriptions?on_conflict=service_name,provider,account_identifier`,
    {
    method: 'POST',
...
export async function updateSubscription(id, patch) {
  checkEnv();
  const res = await fetch(`${SUPABASE_URL}/rest/v1/service_subscriptions?id=eq.${id}`, {
    method: 'PATCH',
    headers: apiHeaders(),
