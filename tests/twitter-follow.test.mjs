...
//   node scripts/run_all.mjs twitter_follow
// This file wraps the same goal as a standalone test.

async function main() {
  const { default: { TRAJECTORIES } } = await import('../scripts/run_all_export.mjs');
  const t = TRAJECTORIES.find(t => t.name === 'twitter_follow');
  if (!t) { console.error('Trajectory not found: twitter_follow'); process.exit(1); }
