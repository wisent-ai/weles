...
//   node scripts/run_all.mjs brightdata_balance
// This file wraps the same goal as a standalone test.

async function main() {
  const { default: { TRAJECTORIES } } = await import('../scripts/run_all_export.mjs');
  const t = TRAJECTORIES.find(t => t.name === 'brightdata_balance');
  if (!t) { console.error('Trajectory not found: brightdata_balance'); process.exit(1); }
