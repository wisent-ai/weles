...
//   node scripts/run_all.mjs instagram_follow
// This file wraps the same goal as a standalone test.

async function main() {
  const { default: { TRAJECTORIES } } = await import('../scripts/run_all_export.mjs');
  const t = TRAJECTORIES.find(t => t.name === 'instagram_follow');
  if (!t) { console.error('Trajectory not found: instagram_follow'); process.exit(1); }
