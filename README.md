...
Read Google 2FA without changing it: configure the Stado-managed operator endpoint with `WELES_WORKER_API_BASE` and `WELES_WORKER_TOKEN`, then use `weles account-security --login-item <skarbiec-item>` and `weles account-security --run <run-id>`. This uses detached `/run` execution and retained diagnostics, not the removed database queue. The [account-security reference](https://weles.wisent.com/docs/account-security) distinguishes a provider-confirmed boolean from an unknown state; a saved seed proves neither.
