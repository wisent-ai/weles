...
import { optionalWelesDatabase } from '../../dist/utils/weles-database.js';
...
// The launcher acquires the database coordinates through their own Skarbiec
// grants and leaves no SUPABASE_* alias in the environment. Both startup
// helpers still read the legacy names, so hand them one derived env view
// instead of exporting a service-role key process-wide.
const welesDatabase = optionalWelesDatabase();
const startupEnv = welesDatabase
  ? { ...process.env, SUPABASE_URL: welesDatabase.url, SUPABASE_SERVICE_ROLE_KEY: welesDatabase.token }
  : process.env;
const databaseCompatibility = await assertDatabaseCompatibility({ env: startupEnv });
...
startDeploymentVersionHeartbeat({ env: startupEnv });
